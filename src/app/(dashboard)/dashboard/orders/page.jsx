...
import { Search, Eye, Truck, CheckCircle, XCircle, ShoppingCart, Check } from "lucide-react"
import { useGetAllOrdersQuery, useUpdateStatusMutation } from "@/redux/features/orderSlice/orderSlice"
...
export default function OrderManagement() {
  ...
  const filteredOrders = orders?.filter((order) => {
    ...
    const customerMatch = order?.userId?.name?.toLowerCase().includes(searchTerm.toLowerCase())
                      ...
                      <Dialog>
                        ...
                        <DialogContent  forceMount className="bg-white">
                          ...
                          <div className="space-y-4 text-sm">
                            <div><strong>Customer :</strong> {order?.userId?.name || "N/A"}</div>
                            <div className="">
                              <strong>Status : </strong> 
                              <form  onSubmit={(e)=>handleStatus(e,order?._id)} className="flex justify-between items-center gap-2 mt-2.5 m-auto w-[90%]">
                                {/* <AntSelect
                                    mode=""
                                ...
                                /> */}
                                <select defaultValue={order?.status} placeholder="update status" name="status" required
                                ...
                                <input type="submit" className="bg-emerald-600 rounded p-3 py-2 text-white" value="update" />
                            ...
                            <div><strong>Item :</strong> 
                              {
                                order?.products?.map((product,idx)=><div className="shadow-sm py-2 px-2 flex justify-between w-[90%] mx-auto" key={idx}>
                                  <Link href={`/products/${product?.productId?._id}`} className="text-sky-600 hover:text-sky-500">{product?.productId?.name}</Link>
                                  <p className="text-">{product?.quantity} Product.</p>
                                </div>)
                              }
                            </div>
