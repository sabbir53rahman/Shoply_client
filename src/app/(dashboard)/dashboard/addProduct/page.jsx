...
const Page = () => {
  ...
  const filteredOptions = allCategory?.filter(
    (item) => !selectedItems.includes(item._id)
  );

  const [disable, setdisable] = useState(false);
  const disableButton =
    "bg-gray-400 hover:bg-gray-400 cursor-pointer text-white w-[80%] font-semibold px-8 py-2.5 rounded-lg shadow-lg transition";
  const normalButton =
    "bg-emerald-600 hover:bg-emerald-700 cursor-pointer text-white w-[80%] font-semibold px-8 py-2.5 rounded-lg shadow-lg transition";
  ...
  const handleAddProduct = async (e) => {
    ...
    try {
      ...
      errorMessage(err?.message || "Failed to add product.");
      setdisable(false);
    }
  };
          ...
          <form onSubmit={handleAddProduct} className="space-y- outline-none">
            ...
            <div className="flex flex-col lg:flex-row gap-4 my-4">
              <div className="w-full">
                ...
                <input
                  type="text"
                  placeholder="Write status... (optional)"
                  name="status"
                  className="w-full border border-gray-300  text-gray-500 rounded-lg px-4 py-2 outline-none"
                />
              </div>
            ...
            <div className="text-center my-10">
              <input
                type="submit"
                disabled={disable}
                className={`${disable ? disableButton : normalButton}  `}
                value="Add Product"
              />
            </div>
          </form>
